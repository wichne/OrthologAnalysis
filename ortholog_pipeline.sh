...
rm nr.all.faa nr.all.faa.clstr all.faa all.faa.* all_v_all.tab mcl_in.abc all.tab all.mci *.ortho synteny_ortho.out all.equivalog.out all.equivalog.tblout multiparanoid.out out.all.mci.I40 $namebase.outfile geneFamily
...
echo "Organism identifiers are drawn from the file prefix of the .gff (or .info) files."

# Catenate the protein fasta files into all.pep
echo "Fasta file is built from .pep files"
cat input/*.faa >> all.faa
...
diamond blastp -d all -q all.faa -f 6 qseqid qlen qstart qend sseqid sseqid slen sstart send bitscore bitscore ppos pident evalue -o all_v_all.tab
...
$ORTHO/multi_bbh.pl -i all_v_all.tab *.gff
...
hmmsearch -o all.equivalog.out --noali --acc --tblout all.equivalog.tblout --cut_tc --cpu 24 /projects/db/hmms/TIGRFAM/equivalog_models.HMM all.faa
...
$ORTHO/build_ortholog_table.pl -namebase $namebase
